{{-- resources/views/components/review-modal.blade.php --}}
@props([
    'modalId',
...
<div id="{{ $modalId }}"
    class="fixed inset-0 bg-black/70 backdrop-blur-sm flex justify-center items-center z-50 p-4 hidden">
    <div class="bg-gray-800 border border-gray-700 rounded-xl shadow-2xl w-full max-w-md transform transition-all">
        <div class="p-6">
            ...
            <form action="{{ $actionRoute }}" method="POST" class="space-y-4">
                @csrf
                ...
                <div>
                    ...
                    <select name="meal_type"
                        class="w-full px-4 py-3 bg-gray-700 border border-gray-600 text-gray-100 rounded-lg focus:ring-2 focus:ring-olive-500 focus:border-olive-500 transition-colors">
                        <option value="breakfast" {{ $defaultMealType == 'breakfast' ? 'selected' : '' }}>🌅 Breakfast
                        </option>
                        <option value="lunch" {{ $defaultMealType == 'lunch' ? 'selected' : '' }}>☀️ Lunch</option>
                ...
                <input type="hidden" name="total_calories" value="{{ $calories }}">
                ...
                <div class="bg-gray-700/50 rounded-lg p-4">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div class="bg-gray-800 p-3 rounded-lg">
                            ...
                            <div class="text-lg font-bold text-olive-400">{{ $calories }}kcal</div>
                        </div>
                        <div class="bg-gray-800 p-3 rounded-lg">
                            <div class="text-xs text-gray-400 uppercase tracking-wide">Protein</div>
                            <div class="text-lg font-bold text-blue-400">{{ $protein }}g</div>
                        </div>
                        <div class="bg-gray-800 p-3 rounded-lg">
                            <div class="text-xs text-gray-400 uppercase tracking-wide">Carbs</div>
                            <div class="text-lg font-bold text-red-400">{{ $carbs }}g</div>
                        </div>
                        <div class="bg-gray-800 p-3 rounded-lg">
                            <div class="text-xs text-gray-400 uppercase tracking-wide">Fat</div>
                            <div class="text-lg font-bold text-yellow-400">{{ $fat }}g</div>
                        </div>
                    </div>
                </div>
